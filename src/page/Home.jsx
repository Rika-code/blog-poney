...
function Home () {
  const poneys = [
   ...
   const MagicPoney = poneys.filter ((poneys) => {
  ...
  const lastFiveMagicPoneys = MagicPoney.slice(-5);
    ...
    <main className='mainHome'>
      {lastFiveMagicPoneys.map((poneys) => {
        return (
          <article className='poneysList'>
            <h2>{poneys.name}</h2>
            <ul className='poneysul'>
              <li> couleur: {poneys.color}</li>
              <li>age : {poneys.age}</li>
            </ul>
            <p><img src ={poneys.img} className='homeImg'></img></p>
          </article>
        )
      }
      )}
